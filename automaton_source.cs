...
namespace Automaton
{
    class Program
    {
        ...
        {
            MooreDiagram m = new MooreDiagram(new Range[] { new Range(0, 1) }, new Range[] { new Range(0, 1) });
            //создаём автомат, который принимает на вход 1 символ принадлежащий [0, 1] и возвращает 1 символ принадлежащий [0, 1]
            //например автомат принимающий 3 символа из [0, 1] и возвращающий 1 символ из [-1, 1] будет создаваться так:
            //MooreDiagram m = new MooreDiagram(new Range[] { new Range(0, 1), new Range(0, 1), new Range(0, 1) }, new Range[] { new Range(-1, 1) });

            //сейчас автомат пустой - у него нет ни одного состояния, а значит диаграмма пустая
            m.SetInitState("0");//задаём начальное состояние автомата. создание автомата всегда начинается с этого метода.
            m.AddNewState("1");
            m.AddNewTransition("0", new AIO(0), "0", new AIO(1));//добавляем стрелочку (a) на диаграмме
            m.AddNewTransition("0", new AIO(1), "1", new AIO(1));//добавляем стрелочку (b) на диаграмме
            m.AddNewTransition("1", new AIO(0), "0", new AIO(0));//добавляем стрелочку (c) на диаграмме
            m.AddNewTransition("1", new AIO(1), "1", new AIO(1));//добавляем стрелочку (d) на диаграмме

            Console.WriteLine(m.Process(new AIO(1)));//подаём в автомат 1
            Console.WriteLine(m.Process(new AIO(0)));//подаём в автомат 0
            Console.WriteLine(m.CurrentState);
            Console.ReadKey();

            //P.S. в автомат можно подавать и больше значений и это будет писаться так: m.Process(new AIO(1, 0, 1, 1))
            //Но в этот автомат так подать не получится, т.к. он принимает на вход только 1 символ
        }
    }

    internal static class Hash
    {
        public static byte[] T = new byte[] { 178, 96, 240, 173, 88, 170, 92, 83, 72, 8, 149, 43, 251, 139, 179, 74, 192, 110, 237, 44, 40, 214, 244, 221, 20, 21, 165, 195, 143, 4, 70, 24, 38, 78, 94, 102, 107, 108, 163, 245, 203, 103, 217, 26, 239, 106, 181, 116, 185, 177, 242, 248, 25, 7, 138, 27, 223, 186, 37, 14, 161, 202, 252, 241, 148, 169, 59, 104, 193, 255, 227, 76, 233, 101, 67, 53, 156, 183, 238, 160, 89, 130, 64, 34, 68, 118, 122, 152, 188, 73, 167, 63, 19, 51, 112, 56, 213, 253, 180, 82, 187, 209, 232, 15, 90, 129, 215, 79, 71, 230, 6, 168, 207, 250, 33, 91, 124, 157, 85, 135, 141, 208, 153, 60, 226, 111, 97, 98, 32, 11, 133, 87, 50, 197, 13, 182, 16, 246, 22, 236, 45, 80, 134, 151, 46, 140, 62, 31, 216, 12, 201, 132, 228, 29, 199, 75, 66, 249, 205, 35, 120, 136, 194, 131, 247, 145, 219, 58, 23, 190, 175, 55, 1, 5, 229, 174, 113, 77, 47, 144, 30, 222, 206, 137, 121, 86, 164, 100, 10, 18, 49, 128, 162, 39, 159, 2, 109, 166, 212, 220, 126, 42, 235, 105, 93, 61, 211, 95, 231, 28, 142, 191, 254, 172, 218, 48, 52, 123, 224, 69, 57, 65, 84, 17, 204, 115, 243, 147, 234, 184, 0, 125, 176, 158, 196, 9, 117, 150, 210, 54, 146, 198, 189, 200, 127, 225, 114, 155, 119, 41, 99, 171, 36, 3, 154, 81 };
